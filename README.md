...
| 별 | **3,853** (3명 이상이 담은 자료) |
| 선 | **868** (3명 이상이 함께 담은 쌍) · 화면에는 골격 474개만 |
| 성단 | 20 |

**모든 별과 선이 "서로 다른 3명 이상" 조건을 만족한다.** 담은 사람이 적은 자료와
두 명만 함께 담은 쌍은 재식별 여지가 있어 공개 데이터에서 제외했다. 이용자 식별자는
애초에 포함되지 않는다 — 집계가 SQL 안에서 끝난다.
...
```bash
python scripts/build_universe.py 3 3           # 이 저장소에 실린 공개판
python scripts/render_universe.py              # public/universe.png

python scripts/build_universe.py               # 내부용 촘촘한 판 (k=2, 기본값)
```

인자는 `[k] [최소 등록자 수] [출력 파일명]` 이다.
...
- **선에 걸리지 않은 별의 의미 있는 좌표.** 공개판에서는 3,853개 중 3,376개가 여기 해당한다.
  선이 없으면 배치 근거가 없어 ID 해시로 흩뿌려져 있다. 가장 큰 미해결 문제다
...
서지정보는 도서관 목록에서 온 사실 정보이고, 등록자 수와 연결은 집계값이다.
개인을 식별할 수 있는 값은 포함되어 있지 않다.

라이선스는 아직 정하지 않았다 — 정해지기 전까지 코드와 데이터의 권리는 유보된다.
